$(document).ready(function (){
  ...
  $.getJSON('../../../../api/analytics.json?dimension=dx:dbpmbNlJShP&dimension=pe:'+periodTypes.join(';')+'&outputIdScheme=NAME&filter=ou:USER_ORGUNIT').done(currentperiods => {
  ...
  $("#getreport").click(function(){
    ...
    var ds = $("#dataset").val();
    var pe = $("#periods").val();
    var reportTypes = [ds+'.EXPECTED_REPORTS', ds+'.ACTUAL_REPORTS', ds+'.REPORTING_RATE', ds+'.ACTUAL_REPORTS_ON_TIME', ds+'.REPORTING_RATE_ON_TIME'];
    
    var baseUrl = '../../../../api/analytics.html?dimension=dx:'; 
    var reporturl = baseUrl+reportTypes.join(';') + '&dimension=ou:'+orgUnit+'&filter=pe:'+pe;
    reporturl += '&columns=dx&rows=ou&tableLayout=true&hideEmptyRows=true&displayProperty=NAME&includeNumDen=false';
    
    $.get(reporturl).done(result => {      
      $('#tableview').empty(); 
      $('#tableview').append("<h3 style='margin-top:0;'>Reporting Rate Summary, <i>"+$('#orgUnitName').val() +"</i>, <i>"+$("#dataset option:selected" ).text()+"</i></h3>"); 
